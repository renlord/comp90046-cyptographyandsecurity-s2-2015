...
class TestDHEXMethods(unittest.TestCase):
	...
	def setUp(self):
		...
		self.public = reference.crypto.dhex.diffie_hellman_pair(self.generator, \
		 	self.prime, self.private)[1]

	@timeout(1)
    def modexp(self, x, y, z):
        return cryptoclient.crypto.dhex.modexp(x, y, z)

    @timeout(1)
    def diffie_hellman_pair(self, g, p, private):
        return cryptoclient.crypto.dhex.diffie_hellman_pair(g, p, private)

    @timeout(1)
    def diffie_hellman_shared(self, private, public, modulus):
        return cryptoclient.crypto.dhex.diffie_hellman_shared(private, public, \
        	modulus)
    ...
    def test_dhPair(self):
        ...
        self.assertEqual(self.diffie_hellman_pair(self.generator, self.prime, \
        	self.private), reference.crypto.dhex.diffie_hellman_pair( \
        	self.generator, self.prime, self.private), err)

    # Diffie Hellman Shared
    def test_dhShared(self):
        err = "FAILED: Shared key computation is incorrect"
        self.assertEqual(self.diffie_hellman_shared(self.private, self.public, \
        	self.prime), reference.crypto.dhex.diffie_hellman_shared( \
        	self.private, self.public, self.prime), err)
