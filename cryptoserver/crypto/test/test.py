import unittest
import random

from timeout import timeout

import part1_crypto
import solution

class TestPart1CryptoFunctions(unittest.TestCase):

    def setUp(self):
        self.dh_g = 
        self.dh_p = 
        self.dh_private = random.getrandbits(2048)
        self.dh_public = solution.diffie_hellman_pair(self.dh_g, self.dh_p, self.dh_private)[1]

        self.dh_ServerPrivate = random.getrandbits(2048)
        self.dh_ServerPublic = solution.diffie_hellman_pair(self.dh_g, self.dh_p, self.dh_ServerPrivate)[1]

    # Candidate Implementations are wrapped around a 1 second timeout decorator just to make sure their implementations are FAST. 
    @timeout(1)
    def modexp(self, x, y, z):
        return part1_crypto.modexp(x, y, z)

    @timeout(1)
    def diffie_hellman_pair(self, g, p, private):
        return part1_crypto.diffie_hellman_pair(g, p, private)

    @timeout(1)
    def diffie_hellman_shared(self, private, public, modulus):
        return part1_crypto.diffie_hellman_shared(private, public, modulus)

    # Modular Exponentiation Test
    def test_modexp_small(self):
        err = "FAILED: Modular Exponentiation for 32bit Integers"
        x = random.getrandbits(32)
        y = random.getrandbits(32)
        z = random.getrandbits(32)
        self.assertEqual(self.modexp(x, y, z), pow(x, y, z), err)

    def test_modexp_medium(self):
        err = "FAILED: Modular Exponentiation for 128bit Integers"
        x = random.getrandbits(128)
        y = random.getrandbits(128)
        z = random.getrandbits(128)
        self.assertEqual(self.modexp(x, y, z), pow(x, y, z), err)

    def test_modexp_large(self):
        err = "FAILED: Modular Exponentiation for 1024bit Integers"
        x = random.getrandbits(1024)
        y = random.getrandbits(1024)
        z = random.getrandbits(1024)
        self.assertEqual(self.modexp(x, y, z), pow(x, y, z), err)

    def test_modexp_real(self):
        err = "FAILED: Modular Exponentiation for 2048bit Integers"
        x = random.getrandbits(2048)
        y = random.getrandbits(2048)
        z = random.getrandbits(2048)
        self.assertEqual(self.modexp(x, y, z), pow(x, y, z), err)

    # Diffie Hellman Pair
    def test_dhPair(self):
        err = "FAILED: Public key computation is incorrect"
        self.assertEqual(self.diffie_hellman_pair(self.dh_g, self.dh_p, self.dh_private), solution.diffie_hellman_pair(self.dh_g, self.dh_p, self.dh_private), err)

    # Diffie Hellman Shared
    def test_dhShared(self):
        err = "FAILED: Shared key computation is incorrect"
        self.assertEqual(self.diffie_hellman_shared(self.dh_private, self.dh_ServerPublic, self.dh_p), solution.diffie_hellman_shared(self.dh_private, self.dh_ServerPublic, self.dh_p), err)

if __name__ == "__main__":
    unittest.main()