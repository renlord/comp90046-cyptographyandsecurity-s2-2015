...
from reference.network.protocol import *
import reference.crypto.dhex
import reference.crypto.stream
import reference.crypto.des
import reference.util.error

CLIENT_CORPUS_PATH = "reference/corpus.txt"
            ...
            try:
                if msg["type"] == "SERVER_DHEX":    
                    ...
                    if "dh_Xc" in msg.keys():
                        ...
                        self.dh_Xc = reference.crypto.dhex.diffie_hellman_private(2048)
                    self.dh_Xc, self.dh_Yc = reference.crypto.dhex.diffie_hellman_pair(self.dh_generator, self.dh_prime, self.dh_Xc)
                    break
            except KeyError:
                print("KeyError: Message does not contain all required fields")
                sys.exit()
        ...
        while True:
            ...
            try:
                if msg["type"] == "SERVER_DHEX_DONE":    
                    self.sharedKey = reference.crypto.dhex.diffie_hellman_shared(self.dh_Xc, self.dh_Ys, self.dh_prime)
                    break
            except KeyError:
                print("KeyError: Message does not contain all required fields")
                sys.exit()
        ...
        while True:
            ...
            try:
                ...
                if msg["type"] == "SERVER_DHEX_ERROR":
                    raise reference.util.error.InvalidDHComputation()
            except KeyError:
                print("KeyError: Message does not contain all required fields")
                sys.exit()     
            except reference.util.error.InvalidDHComputation as e:
                print(e.errorMessage())
                sys.exit()

    def communication_phase(self):
        # Instantiation of Ciphers.crypto.stream.StreamCipher(self.sharedKey, self.dh_prime, self.streamKeys[0], self.streamKeys[1])
        self.streamCipher = reference.crypto.stream.StreamCipher(self.sharedKey, self.dh_prime, self.streamKeys[0], self.streamKeys[1])
